...
const ExpertBookingModal = ({ open, onClose, expert }) => {
    ...
    const fetchBlockedSlots = async (date) => {
        try {
            const res = await axios.get(
                `http://localhost:8000/bookings/blocked-slots?expertId=${expert.expertId}&date=${date}`
            );
